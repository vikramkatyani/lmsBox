...
public partial class CoursesController : ControllerBase
{
    ...
    public async Task<IActionResult> GetCertificate(string courseId)
    {
        try
        {
            ...
            {
                return BadRequest(new { message = "Certificates are not enabled for this course." });
            }
            ...
            if (!CourseCompletionHelper.IsCourseCertificateEligible(
                    course,
                    course.Lessons.ToList(),
                    lessonProgresses,
                    courseProgress))
            {
                var hasPractical = course.Lessons.Any(CourseCompletionHelper.IsExternalLesson);
                var message = hasPractical
                    ? "Certificate unlocks after the post-course survey is completed and the practical is passed."
                    : "Complete all course requirements before requesting a certificate.";
    ...
    public async Task<ActionResult<CourseDetailDto>> GetCourseDetail(string courseId)
    {
        try
        {
            ...
            // Get course with lessons and post-survey metadata for sidebar display
            var course = await _context.Courses
            ...
            var certificateEligible = CourseCompletionHelper.IsCourseCertificateEligible(
                course,
                orderedLessons,
                lessonProgresses,
                courseProgress);
            var postSurveyUnlocked = CourseCompletionHelper.AreLessonsReadyForPostSurvey(
                orderedLessons,
                lessonProgresses);
            ...
            var lessonsLocked = CourseLessonAccessHelper.AreLessonsLockedByPreSurvey(course, courseProgress);
            var lessonCompletionById = CourseLessonAccessHelper.BuildLessonCompletionMap(lessonProgresses);
            ...
            var courseDetail = new CourseDetailDto
            {
                ...
                PostSurveyUnlocked = postSurveyUnlocked,
                ...
                Lessons = course.Lessons.Select(lesson =>
                {
                    ...
                    var isLocked = CourseLessonAccessHelper.IsLessonLocked(
                        course,
                        lesson,
                        orderedLessons,
                    ...
                    if (string.Equals(lesson.Type, "interactive", StringComparison.OrdinalIgnoreCase) &&
                        !InteractiveLessonHelper.IsUsableForLearners(lesson.InteractiveLessonSettings?.Blocks ?? []))
                    {
                        isLocked = true;
                    }
                    ...
                    if (lesson.Type.ToLower() == "video" && !string.IsNullOrEmpty(lesson.CaptionUrl))
                    {
                        ...
                        if (_blobService.IsConfigured() && captionUrl.Contains("blob.core.windows.net"))
                        {
                            ...
                            {
                                _logger.LogWarning(ex, "Failed to generate SAS URL for caption on lesson {LessonId}, URL: {Url}. Using original URL", lesson.Id, captionUrl);
                            }
                        }
                    }
                    ...
                    return new LessonDto
                    {
                        ...
                        PracticalStatus = string.Equals(lesson.Type, "external", StringComparison.OrdinalIgnoreCase)
                            ? lessonProgress?.ScormLessonStatus
                            : null
                    };
                }).ToList()
            };
    ...
    public async Task<IActionResult> UpdateLessonProgress(string courseId, long lessonId, [FromBody] UpdateProgressRequest request)
    {
        try
        {
            ...
            var accessError = await ValidateLessonAccessAsync(userId, courseId, lessonId);
            if (accessError != null)
            {
                return accessError;
            }

            var lessonMeta = await _context.Lessons
                .AsNoTracking()
            ...
            if (string.Equals(lessonMeta.Type, "external", StringComparison.OrdinalIgnoreCase) && request.Completed)
            {
                return BadRequest(new { message = "External lessons cannot be completed from within the LMS." });
            }
...
public partial class CoursesController
{
    ...
    public async Task<IActionResult> GetPostCourseSurvey(string courseId)
    {
        try
        {
            ...
            // Check if all lessons are ready for post-survey (practical passed or failed counts)
            var lessons = await _context.Lessons
                .Where(l => l.CourseId == courseId)
                .ToListAsync();
            var lessonProgressesForSurvey = await _context.LearnerProgresses
                .Where(lp => lp.UserId == userId && lp.CourseId == courseId && lp.LessonId != null)
                .ToListAsync();

            if (!CourseCompletionHelper.AreLessonsReadyForPostSurvey(lessons, lessonProgressesForSurvey))
            {
                var completedLessonsReady = lessons.Count(l =>
                    CourseCompletionHelper.IsLessonSatisfiedForSurveyUnlock(
                        l,
                        lessonProgressesForSurvey.FirstOrDefault(lp => lp.LessonId == l.Id)));

                return BadRequest(new
                {
                    error = "All lessons must be completed before accessing the post-course survey",
                    completedLessons = completedLessonsReady,
    ...
    public async Task<IActionResult> SubmitPostCourseSurvey(string courseId, [FromBody] SubmitSurveyRequest request)
    {
        try
        {
            ...
            // Verify lessons are ready for post-survey (practical passed or failed counts)
            var lessonsForSubmit = await _context.Lessons
                .Where(l => l.CourseId == courseId)
                .ToListAsync();
            var lessonProgressesForSubmit = await _context.LearnerProgresses
                .Where(lp => lp.UserId == userId && lp.CourseId == courseId && lp.LessonId != null)
                .ToListAsync();

            if (!CourseCompletionHelper.AreLessonsReadyForPostSurvey(lessonsForSubmit, lessonProgressesForSubmit))
    ...
    private async Task<IActionResult?> ValidateLessonAccessAsync(string userId, string courseId, long lessonId)
    {
        ...
        var courseProgress = await _context.LearnerProgresses.AsNoTracking()
            .Where(lp => lp.UserId == userId && lp.CourseId == courseId && lp.LessonId == null)
            .OrderByDescending(lp => lp.ProgressPercent)
            .FirstOrDefaultAsync();

        var lessonProgresses = await _context.LearnerProgresses.AsNoTracking()
            .Where(lp => lp.UserId == userId && lp.CourseId == courseId && lp.LessonId != null)
            .ToListAsync();
        ...
        if (!CourseLessonAccessHelper.IsLessonLocked(course, lesson, orderedLessons, lessonCompletionById, courseProgress))
        {
            return null;
        }

        if (CourseLessonAccessHelper.AreLessonsLockedByPreSurvey(course, courseProgress))
        {
            return StatusCode(403, new { message = "Complete the pre-course survey before accessing lessons." });
        }

        return StatusCode(403, new { message = "Complete the previous lesson before accessing this content." });
    }
}
...
// Course resources endpoints for learners
public partial class CoursesController
{
    /// <summary>
    /// Get supplementary resources linked to a course for the current learner.
    /// </summary>
    [HttpGet("{courseId}/resources")]
    public async Task<ActionResult<List<LearnerCourseResourceDto>>> GetCourseResources(string courseId)
    {
        try
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            ...
            if (!await LearnerHasCourseAccessAsync(userId, courseId))
            {
                return Forbid();
            }
            ...
            var result = new List<LearnerCourseResourceDto>();
            foreach (var resource in resources)
            {
                result.Add(new LearnerCourseResourceDto
                {
                    Id = resource.Id,
                    Title = resource.Title,
                    Description = resource.Description,
                    Type = resource.Type,
                    Url = await ResolveLearnerResourceUrlAsync(resource),
                    ThumbnailUrl = await ResolveLearnerThumbnailUrlAsync(resource)
                });
            }
        ...
        {
            _logger.LogError(ex, "Error fetching resources for course {CourseId}", courseId);
            return StatusCode(500, new { message = "An error occurred while fetching course resources" });
        }
    }

    /// <summary>
    /// Get a single course resource with SAS URLs applied (for inline HTML content).
    /// </summary>
    [HttpGet("{courseId}/resources/{resourceId}")]
    public async Task<ActionResult<LearnerCourseResourceDto>> GetCourseResource(string courseId, long resourceId)
    {
        try
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            ...
            return Ok(new LearnerCourseResourceDto
            {
                Id = resource.Id,
                Title = resource.Title,
                Description = resource.Description,
                ...
                Url = await ResolveLearnerResourceUrlAsync(resource),
                ThumbnailUrl = await ResolveLearnerThumbnailUrlAsync(resource),
                HtmlContent = string.IsNullOrEmpty(resource.HtmlUrl) ? resource.HtmlContent : null
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching resource {ResourceId} for course {CourseId}", resourceId, courseId);
            return StatusCode(500, new { message = "An error occurred while fetching the resource" });
        }
    }

    private async Task<bool> LearnerHasCourseAccessAsync(string userId, string courseId)
    {
        var hasAccessThroughGroup = await _context.LearnerGroups
            .Where(lg => lg.UserId == userId && lg.IsActive)
            .Join(_context.GroupCourses, lg => lg.LearningGroupId, gc => gc.LearningGroupId, (lg, gc) => gc)
        ...
        return await _context.LearnerPathwayProgresses
            .Where(lpp => lpp.UserId == userId)
            .Join(_context.PathwayCourses, lpp => lpp.LearningPathwayId, pc => pc.LearningPathwayId, (lpp, pc) => pc)
            .AnyAsync(pc => pc.CourseId == courseId);
    }

    private async Task<string?> ResolveLearnerResourceUrlAsync(CourseResource resource)
    {
        var rawUrl = resource.Type.ToLower() switch
        {
            "pdf" => resource.DocumentUrl,
            "video" => resource.VideoUrl,
            "html" => resource.HtmlUrl,
            _ => null
        };
        ...
        if (_blobService.IsConfigured() && rawUrl.Contains("blob.core.windows.net"))
        {
            try
            {
                return await _blobService.GetSasUrlAsync(rawUrl, 24);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to generate SAS URL for resource {ResourceId}", resource.Id);
            }
        }
    ...
    private async Task<string?> ResolveLearnerThumbnailUrlAsync(CourseResource resource)
    {
        ...
        if (_blobService.IsConfigured() && resource.ThumbnailUrl.Contains("blob.core.windows.net"))
        {
            try
            {
                return await _blobService.GetSasUrlAsync(resource.ThumbnailUrl, 24);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to generate SAS URL for resource thumbnail {ResourceId}", resource.Id);
            }
        }
...
public class LearnerCourseResourceDto
{
