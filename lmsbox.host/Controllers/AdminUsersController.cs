                ...
                if (!string.IsNullOrEmpty(roleToAssign))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (FavoriteReportDefaults.TryApplyDefaults(user, roleToAssign))
                            {
            ...
            try
            {
                ...
                foreach (var email in emailList)
                {
                    try
                    {
                        ...
                        if (roleAssigned && FavoriteReportDefaults.TryApplyDefaults(user, roleToAssign))
                        {
                            await _userManager.UpdateAsync(user);
                        }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(request.Role))
                {
                    ...
                    if (!currentRoles.Contains(newRole))
                    {
                        ...
                        if (FavoriteReportDefaults.TryApplyDefaults(user, newRole))
                        {
                            await _userManager.UpdateAsync(user);
                        }
                    }
