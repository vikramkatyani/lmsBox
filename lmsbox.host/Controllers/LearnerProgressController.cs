...
public class LearnerProgressController : ControllerBase
{
        ...
        [FromBody] UpdateLessonProgressRequest request)
    {
        try
        {
            ...
            var accessError = await ValidateLessonAccessAsync(userId, lesson);
            if (accessError != null)
            {
                return accessError;
            }
    ...
    private async Task<ActionResult?> ValidateLessonAccessAsync(string userId, Lesson lesson)
    {
        var course = lesson.Course ?? await _context.Courses
            .Include(c => c.Lessons)
            .FirstOrDefaultAsync(c => c.Id == lesson.CourseId && !c.IsDeleted);
        ...
        var courseProgress = await _context.LearnerProgresses.AsNoTracking()
            .Where(lp => lp.UserId == userId && lp.CourseId == lesson.CourseId && lp.LessonId == null)
