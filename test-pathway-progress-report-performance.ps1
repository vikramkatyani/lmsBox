...
$activeOnlyPart = "&activeOnly=$ActiveOnly"

$summaryUrl = "$BaseUrl/api/admin/reports/pathway-progress/summary?$($activeOnlyPart.TrimStart('&'))"
$pathwaysUrl = "$BaseUrl/api/admin/reports/pathway-progress/pathways?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$activeOnlyPart"
$combinedUrl = "$BaseUrl/api/admin/reports/pathway-progress?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$activeOnlyPart"

Write-Host "Summary URL : $summaryUrl" -ForegroundColor DarkGray
Write-Host "Pathways URL: $pathwaysUrl" -ForegroundColor DarkGray
Write-Host "Iterations  : $Iterations" -ForegroundColor DarkGray
Write-Host "Concurrency : $Concurrency" -ForegroundColor DarkGray

Invoke-Benchmark -Name "Pathway Progress Summary" -Url $summaryUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
Invoke-Benchmark -Name "Pathway Progress Pathways" -Url $pathwaysUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency

if ($IncludeCombined) {
    Invoke-Benchmark -Name "Pathway Progress Combined" -Url $combinedUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
}
