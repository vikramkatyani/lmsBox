...
if (-not [string]::IsNullOrWhiteSpace($CourseId)) {
    $parts += "courseId=$([uri]::EscapeDataString($CourseId))"
}
if (-not [string]::IsNullOrWhiteSpace($Status)) {
    $parts += "status=$([uri]::EscapeDataString($Status))"
}
...
$summaryUrl = "$BaseUrl/api/admin/reports/user-course-progress/summary"
if (-not [string]::IsNullOrWhiteSpace($commonQuery)) {
    $summaryUrl = "$summaryUrl?$commonQuery"
}

$recordsQuery = @("pageNumber=1", "pageSize=$PageSize", "sortBy=$SortBy", "sortDirection=$SortDirection")
if (-not [string]::IsNullOrWhiteSpace($commonQuery)) {
    $recordsQuery += $parts
}
$recordsUrl = "$BaseUrl/api/admin/reports/user-course-progress/records?$($recordsQuery -join '&')"

$combinedUrl = "$BaseUrl/api/admin/reports/user-course-progress?$($recordsQuery -join '&')"

Write-Host "Summary URL : $summaryUrl" -ForegroundColor DarkGray
Write-Host "Records URL : $recordsUrl" -ForegroundColor DarkGray
Write-Host "Iterations  : $Iterations" -ForegroundColor DarkGray
Write-Host "Concurrency : $Concurrency" -ForegroundColor DarkGray

Invoke-Benchmark -Name "User-Course Progress Summary" -Url $summaryUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
Invoke-Benchmark -Name "User-Course Progress Records" -Url $recordsUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency

if ($IncludeCombined) {
    Invoke-Benchmark -Name "User-Course Progress Combined" -Url $combinedUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
}
