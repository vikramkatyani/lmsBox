...
$summaryUrl = "$BaseUrl/api/admin/reports/course-enrollment/summary"
$coursesUrl = "$BaseUrl/api/admin/reports/course-enrollment/courses?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$categoryPart"
$combinedUrl = "$BaseUrl/api/admin/reports/course-enrollment?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$categoryPart"
...
Run-Benchmark -Name "Course Enrollment Summary" -Url $summaryUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
Run-Benchmark -Name "Course Enrollment Courses" -Url $coursesUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency

if ($IncludeCombined) {
    Run-Benchmark -Name "Course Enrollment Combined" -Url $combinedUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
}
