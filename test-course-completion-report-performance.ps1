...
if (-not [string]::IsNullOrWhiteSpace($Category)) {
    $categoryPart = "&category=$([uri]::EscapeDataString($Category))"
}

$performancePart = ""
if (-not [string]::IsNullOrWhiteSpace($Performance)) {
    $performancePart = "&performance=$([uri]::EscapeDataString($Performance))"
}

$summaryUrl = "$BaseUrl/api/admin/reports/course-completion/summary"
$coursesUrl = "$BaseUrl/api/admin/reports/course-completion/courses?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$categoryPart$performancePart"
$combinedUrl = "$BaseUrl/api/admin/reports/course-completion?pageNumber=1&pageSize=$PageSize&sortBy=$SortBy&sortDirection=$SortDirection$searchPart$categoryPart$performancePart"

Write-Host "Summary URL : $summaryUrl" -ForegroundColor DarkGray
Write-Host "Courses URL : $coursesUrl" -ForegroundColor DarkGray
Write-Host "Iterations  : $Iterations" -ForegroundColor DarkGray
Write-Host "Concurrency : $Concurrency" -ForegroundColor DarkGray

Run-Benchmark -Name "Course Completion Summary" -Url $summaryUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
Run-Benchmark -Name "Course Completion Courses" -Url $coursesUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency

if ($IncludeCombined) {
    Run-Benchmark -Name "Course Completion Combined" -Url $combinedUrl -Headers $headers -Iterations $Iterations -Concurrency $Concurrency
}
